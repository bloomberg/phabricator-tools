...
class PhlsysSubprocessTests(unittest.TestCase):
    ...
    def test_invalid_kwargs(self):
        ...
        self.assertRaises(AssertionError, run, workingDir='foo', stdin='bar', unexpected='foo')
    ...
    def test_run_multi_commands(self):
        ...
        self.assertEqual(None, runCommands("echo hello stdout", "echo goodbye stdout"))

    def test_assertion_raised(self):
        "Assertion raised on bad character input"
        for bad in ("'", '"', '`'):
            self.assertRaises(AssertionError, runCommands, "echo {0}hello stdout{0}".format(bad))
